...
Indeed, a constant temperature is maintained when the kinetic energy is constant on average, while the pressure is given by the average quantity of motion exchanged between the particles and the walls of the box they are confined to, which depends on the frequency of collision and thus on the extent of the box. As most MD simulations are run under periodic boundary conditions, i.e. a particle which exits from the simulation box during a move is brought back on the opposite side of the box, to mimic the presence of an infinite number of equivalent boxes one next to the other, particles are not bouncing on the box walls, rather a virtual pressure is computed from the velocities of the ones trespassing the box boundaries during a move.

The thermostat algorithms used in this work are the Berendsen \cite{Berendsen1984} and velocity rescale \cite{Bussi2007}, which differ the way they rescale velocities to grant the correct average kinetic energy, while the barostat, which rescale the simulation box to influence the pressure are the Berendsen \cite{Berendsen1984} and Parrinello-Rahman \cite{Parrinello1981} one.
