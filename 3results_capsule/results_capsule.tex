...
\item To evaluate the mobility of lipids on the membrane plane, for each simulation we extracted the trajectory of the phosphorus atom of every lipid, in the top and bottom leaflet separately, removing the collective motion of the leaflet with respect to the simulation box frame. The Mean Square Displacement of each lipid is computed as a function of time (MSD(t)). For a given time $t$, the MSD is obtained for all the possible time windows of that length fitting within the simulation time analysed and the curves averages. These curves are averaged also over all the lipids of a given species belonging to the leaflet of interest. The diffusion coefficient D is obtained from a linear fit of the average MSD profile, following Einstein equation in two dimensions \citep{Einstein1956}:
\begin{equation}
\langle \left( x - x_0 \right)^2 \rangle = 4Dt.
\end{equation}
The fit was performed in the regions which showed a linear dependence. This implied discarding the first interval of the profile, where the behaviour is not linear, and the last one, where the poorer statistics leads to more noisy data, 50 ns each for atomistic simulations and 100 ns each for MARTINI ones (values chosen to be suitable for each simulation). Moreover the profile was computed discarding the first part of the trajectory, the exact amount of which depends on the system, and it is chosen monitoring the area per lipid.
%
...
We now discuss the simulations of capzip in contact with a membrane. First we focus on atomistic simulations of the pentagonal peptidic subunit in contact with a model bacterial or mammal membrane, both under standard simulation conditions and with an applied external electric field. In this we elucidate the local effect the peptide has on the lipid organisation.
%
...
The difference can be attributed to the force field or to the size of the patch. Extensive tests on many phosphocholine lipids (see Chapter \ref{chapter:lip_par}) suggests that generally 54a8 gives larger ApL values than the ones computed with 54a7.
%
Therefore we attribute the discrepancy partly to reproducibility issues, partly to the different sizes of the patches. Later in this section other cases are reported in which the size has a great influence on the simulation outcome.

Similarly, the larger diffusion coefficient observed for the large membrane can be attributed to the effect of periodic boundary conditions. It is well known that they affect considerably the computed diffusion coefficient \citep{Camley2015,Venable2017}. Specifically, for standard atomistic simulations of the size employed here, this is systematically underestimated. Therefore increasing the size would increase the coefficient, toward its real value (which is obtained for an infinite box).
%
This is indirectly confirmed by simulations of monolipid patches (see Chapter \ref{chapter:lip_par}) which show very similar values of D for simulations run with 54a7 or 54a8, when keeping all the other conditions identical.
...
%
Consistently, the diffusion coefficient D had a 6-fold reduction for both species of lipids (Figure \ref{fig:lip_diff}). The order parameter S$^{avg}_{CD}$ increased slightly and (Figure \ref{fig:lip_ord}). The variability on S$^{avg}_{CD}$ is high, showing that some regions reached a value larger than 0.3, which is often associated to transition to the gel phase, characterised by highly order lipid chains \citep{Pluhackova2016}.
%
...
%
So, to understand the local determinants of this outcome, we computed the diffusion coefficient for subsets of lipids based on their distance from the peptide at the initial frame of the production run (Table \ref{table:D_space}).
%
All lipids are slowed down in their diffusion, but the ones closer to the protein are more affected. Values of the diffusion coefficient smaller than 1 $\mu$m/s$^2$ (as the ones observed for the lipids within 1 nm from the peptide) are usually associated to the gel phase \citep{Scomparin2009}. Clearly, the membrane is not in such phase, as the ApL is larger and allows for disorder of the tail. However, it is interesting that a practical freezing of the lipids happens as they are impeded in their lateral movement.
%
\begin{figure}[t!]
\centering
\vspace{1cm}
...
This effect is due to electrostatic and hydrogen bond interactions between the lipids and the peptide. We computed their number, monitoring the ones which persisted more than 50\% of the time (Table \ref{table:hb_pr_lip}). Moreover, we singled out the ones involving Arginine and Tryptophan residues.
%
...
The properties of the stand alone membrane are computed as before and compared with the experimental data, which are available for this membrane. The ApL is 0.592(3) nm$^2$: the experimental range at 303 K of temperature is 0.608-0.632 nm$^2$ (see Table 1 in \citep{Poger}). These experiments are performed on a variety of lipid geometries, from vesicle to supported lipid bilayers (resting on a solid surface), justifying the broad range of value found. The ApL found in the simulation is smaller than the experimental range. However, the membrane is simulated in presence of a 150 mM salt concentration, while the experiments do not adopt it. As discussed extensively in \citet{Reif2017}, several computational studies report a reduction of area per lipid when salt concentration is introduced \citep{Reif2017, Bockmann2003,Jarerattanachat2013}. This is consistent with the experimental evidence \citet{Pabst2007}, even though simulations are often overestimating the variation of ApL due to the salt \citep{Reif2017}.
%
To support this, analogous simulations run on a 512 lipid patch of DLPC, without salt, gave an ApL of 0.626(5) nm$^2$. The set up for this simulations, run in the context of the parametrisation work presented in Chapter \ref{chapter:lip_par}, is not identical to the one chosen for the present simulations, however control runs show that the difference in the set up is unlikely to affect the ApL to the extent observed. Therefore, we conclude that the effect of the salt concentration is responsibe of the membrane shrinking.

another simulation on a 512 patch, with the 

 without any salt concentration added to the solution. 

0.626 sim 512
0.626(5) 54a8 512 no salt

(0.608$\,\pm\,$0.012 nm$^2$ for DLPC \citep{Kucerka2011} 
...
\subsection{Coarse grain simulations of the buckyball on model membranes} \label{sec:results_lip_cg}
...
\begin{tabular}{lccccc}
 ...
 \hline
\end{tabular}
\captionof{table}{Table of control simulations of membranes complexes. All run at 150 mM concentration of NaCl. Force fields (FF): GR = united atom GROMOS, MA = coarse-grain MARTINI, MA\_P = coarse-grain MARTINI with polar water. Superscript P refers to the rupture of the membrane (poration).}
