...
\begin{tabular}{l|ccc}
 ...
 Buckyball bilayer \& monolayer (120) & MA\_P & 1000 & 2 \& \\
...
%
The SIRAH simulated bilayer capsule has a more open structure, with a skewed and broader radial density profile, while MARTINI with the standard water model provides a more compact configuration (respectively 6.0 nm and 4.6 nm average radius, with 2.9 nm and 1.9 nm average thickness - see Figure \ref{fig:Rg} and \subcaptionref{fig:RDF}). MARTINI runs with the polar water model provide a slightly larger structure with respect to standard MARTINI (4.8 nm average radius), with a comparable thickness. This is likely due to the poorer properties of solvation of standard MARTINI water, which cause the protein beads to preferentially interact between themselves rather than with the solvent. 
...
%
\textbf{[redo on half of simulation?]} The correlation effects are much more pronounced in standard MARTINI, were a greater contraction at the beginning of the simulation results in a smaller object with the two layers strongly correlated in their motion.
...
%
The MARTINI simulations show a lower number of contacts between molecules belonging to the same layer, probably due to the less specific interactions between different beads which do not allow a description of the hydrogen bond like interactions. Moreover, MARTINI results show that the structure obtained after a longer time scale is less organised, but does keep its overall shape thanks to the tight packing between chains. Consistently with this and the smaller thickness of the shell in the MARTINI simulations, the number of contacts between the two layers is higher than in the atomistic simulation. \textbf{[CHECK]}
...
Finally, the values of SASA cannot be compared between force fields, because of the different dimensions and number of atoms/beads employed. However, it is interesting to notice that consistently across force fields, Arginine constitute around half of the exposed surface, with the exception of SIRAH force field, where the more open structure results in the other residues to be more exposed (Figure \ref{fig:all_sasa}).

Overall, these results suggest that every coarse grain force field has a particular propensity for an equilibrium distance between peptidic components due to the different property of solvation of the water model chosen. To better understand this, we computed the energy Coulomb and Lennard-Jones contribution due to peptide-peptide interactions or peptide-solvent one.
%
...
%
A great difference can be seen among force field, as expected. For example standard MARTINI has Coulomb components much closer to zero with respect to polar MARTINI, due to the fact that the dielectric constant is higher and thus these interactions are decreased.

\begin{figure}[p!]
\centering
\subbottom[]{\includegraphics[width=\linewidth]{3results_capsule/pics/many_energies.png} \label{fig:value_eng_cg}} 
\subbottom[]{\includegraphics[width=\linewidth]{3results_capsule/pics/ratio_energies.png} \label{fig:ratio_eng_cg}} 
\caption[bb]{(a) }
\label{fig:eng_cg}
\end{figure}

A more informative comparison instead is given by the ratio between the Protein-Protein and Protein-Water components, for both Coulomb and Lennard-Jones. Figure \ref{fig:ratio_eng_cg} shows that the ratio of the Lennard-Jones interactions shows remarkable agreement between the two MARTINI models, and a similar value to the SIRAH one.
%
For the Coulomb interactions, all the coarse grain models propose a ratio between the Protein-Protein and the Protein-Water component of about 3 to 4, with Polar MARTINI having a greater Protein-Protein component with respect to standard MARTINI. This seems at odd with the previous conclusion that Polar MARTINI produces less compact structures, as a lower Protein-Protein energy should favour protein contacts. However, this is likely compensated by the lower Water-Water interaction, which promotes Protein-Water contacts.
% 
An interesting follow up on this topic would be investigating wonder whether a tuning of the dielectric constant used in either cases can produce more consistent results between the two water model. The constant however was chosen to reproduce at best the properties of bulk water and solvation free energies of ions for both cases, raising the question whether the protein parametrisation performs equally good in conjunction with both model, or which one is more compatible.

Finally, to be noticed that in the SIRAH force field, the Coulomb interactions have a greater magnitude with respect to the Lennard-Jones ones (for both Protein-Protein and Protein-Water interactions), contrary to the MARTINI models. This can be due to the more explicit parametrisation of the side chains of amino acid, while in MARTINI a beads include more atoms, ruling out some of the electrostatics.



\paragraph{Bilayer versus monolayer capsules}
To prove also on large scale objects that the bilayer structure is indeed essential to grant a structure which does not disassemble or change shape, we perform simulations of a monolayer capsule (specifically taking the external layer of the capsule already simulated) in all the three coarse grain force fields employed so far.

The RMSD with respect to the initial configuration \textbf{[REDO for all]} shows that the monolayer structure undergoes a larger conformational change than the bilayer. Consistently, the RDF profile shows that the object contracts much more than the bilayer in both the MARTINI simulations. Likely, this brings the hydrophobic component closer and thus help their screening from the solvent. In the SIRAH simulations instead, the balance of the forces is not sufficient to cause a contraction of the whole structure. Therefore, a rearrangement of nearby branches is observed which brings the Tryptophan side chains closer (Figure --). Consistently, the SASA of Tryptophan residues decreases \textbf{[DO]} (Figure --).
