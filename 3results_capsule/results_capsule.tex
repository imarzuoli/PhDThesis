...
The measure of paired branches in the buckyball network, shown in Figure \ref{fig:BTI_beta}, reports an average (in time) of 240 pairings between arms belonging to the same layer (summing over inner and outer), four pairing per molecule, and around 60 only for inter-layer ones.
%
\begin{figure}[p]
\centering
\includegraphics[width=0.85\linewidth]{3results_capsule/pics/stAll_beta_R1.png}
\caption[Branch pairing during simulations of the buckyball]{\textbf{REPUT BACKMAPPING?, BETTER} Number of paired branches within the same layer and between layers, as defined in the main text. Results are shown for Replica 1 of each simulation set up.}
\label{fig:BTI_beta}
\end{figure}
%
...
\caption[Contacts between molecules during simulations of the buckyball]{\textbf{REPUT BACKMAPPING?, BETTER} Contacts with persistence greater than 50\% between C$_\alpha$s (top row), or between side chains (bottom row) and different simulations set up (Replica 1 for each of them). A similar plot for mixed C$_\alpha$-side chain contact in shown in Figure -- of Appendix --.}
\label{fig:BTI_cont}
\end{figure}
